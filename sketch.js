...
function setup() {
    ...
    road = createSprite(width-500,height-500,500,500);
    ...
    player = createSprite(width/6,height-100);
    player.addAnimation("player",playerImg);
    player.scale =0.2

    ghost = createSprite(width/20,height/2+300);
    ghost.addImage(ghost_img);
    ghost.scale =0.4

    invisibleGround = createSprite(width/6,height+40);
...
function draw() {
   ...
   if (road.x <150){
    ...
    if (fireGroup.isTouching(player)||bombGroup.isTouching(player)){
        ...
        stroke("gold");
        strokeWeight(10);
        text("GAME OVER",width/18,height/2+100)
    }
 ...
 textSize(80);
fill("black");
text("Treasure: "+ treasure ,width/2,height/8);
}

function spawnObstacleFire() {
   if(frameCount%500 ===0){
   fire = createSprite(width,height-100 );
   fire.velocityX =-30
...
function spawnObstacleBomb(){
   ...
   bomb = createSprite(width,height-100);
   bomb.velocityX =-30
